...
public class Host extends Person{
    ...
    private HashMap<Critere, String> criteres ;

    public Host(String nom, String prenom, LocalDate dNaiss, Pays pays, ArrayList<String> hobbies, HashMap<Critere, String> criteres, ArrayList<LocalDate> historique) {
        super(nom, prenom, dNaiss, pays, hobbies, historique);
        this.criteres = criteres;
    }
    public Host(String nom, String prenom, LocalDate dNaiss, Pays pays) {
    ...
    public boolean isCompatible(Person p){
        if (p.getClass() == Host.class) {
            return false; // ne pas apparier deux hôtes
        }
        for (Critere c : criteres.keySet()) {
            switch (c.name()){
                case "HOST_HAS_ANIMAL":
                    if (p.getCriteres().containsKey(Critere.GUEST_ANIMAL_ALLERGY) &&
                        p.getCriteres().get(Critere.GUEST_ANIMAL_ALLERGY).equals("true")){
                        return false;
                    }
                case "HISTORY":
    ...
    public int calculerAffinite(Person p){
        ...
        for (Critere c : criteres.keySet()) {
            switch (c.name()){
                ...
                case "HOST_FOOD":
                    if (p.getCriteres().containsKey(Critere.GUEST_FOOD) &&
                        !p.getCriteres().get(Critere.GUEST_FOOD).equals(p.getCriteres().get(Critere.HOST_FOOD))){
